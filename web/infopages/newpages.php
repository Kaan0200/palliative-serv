...
if (isset($_POST['Submit'])) {
	...
	if ($titleErr == "" and $textErr == "" and $textDetErr == "" and $linktextErr == "") {
		...
		$con = get_connection();
		$sql = "CALL Insert_Page(".$parent_id.",\"".$title."\", \"".$text."\", \"".$detailtext."\", \"".$linktext."\");";
		error_log("-----".$sql."-----");
		mysqli_query($con, $sql);
		mysqli_close($con);
		echo "<h3 style=\"color:green\">Page successfully created!</h3>";
	} else {
		echo $parenterr;
		echo $titleErr;
		echo $textErr;
...
<div>
	<div>
		...
		<div class="InputColumn">
			<input type="text" name="title" style="width:100%" autocomplete="off" value="<?php echo $_POST["title"];?>"></input>
		</div> 
		<br>
		...
		<div class="InputColumn">
			<input type="text" name="linktext" style="width:100%" autocomplete="off" value="<?php echo $_POST["linktext"];?>"></input>
		</div>
	</div>
</div>
